@extends("admin.admin_master")

@section("admin")
 
    <div class="row">
       
        <div class="col-lg-12">
            <div class="card card-default">
                
                <h2 class="card-header  card-header-border-bottom">
                    All Slider &nbsp; <span style="color: red;">|</span> &nbsp; <a class="float-right" href="{{ route('create.slider') }}">Add Slider</a>
                </h2>
               
                <div>
                    @if(session("success"))

                        <div class="alert alert-success alert-dismissible fade show" role="alert">
                            <strong>{{ session("success") }}!</strong>
                            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                        </div>

                    @endif
                   
                </div>

                <div class="card-body">
                    <div class="table-responsive-sm">
                        <table class="table">
                            <thead>
                                <tr>
                                    <th scope="col">S/N</th>
                                    <th scope="col">Image</th>
                                    <th scope="col">Title</th>
                                    <th scope="col">Content</th>
                                    <th scope="col">Created_at</th>
                                    <th scope="col">Updated_at</th>
                                    <th scope="col"></th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach($sliders as $key => $slider)
                                <tr>
                                    <th scope="row">{{ $sliders->firstItem() + $loop->index }}</th>
                                    <td>
                                        <img 
                                            src="{{ !empty($slider->slider_image) ? asset($slider->slider_image) : ' https://via.placeholder.com/70x40.png' }}" 
                                            width="70px"
                                            height="40px"
                                            alt="brand"
                                        >
                                    </td>

                                    <td>{{  $slider->slider_title }}</td>
                                    <td>{!! Str::limit($slider->slider_description, 50) !!}</td>
                                    
                                    
                                    <td> 
                                        @if($slider->created_at == NULL) 
                                            <span>No Date Set</span>
                                        @else
                                            {{ Carbon\Carbon::parse($slider->created_at)->diffForHumans() }}
                                        @endif
                                    </td>

                                    <td> 
                                        @if($slider->updated_at == NULL) 
                                            <span>No Date Set</span>
                                        @else
                                            {{ Carbon\Carbon::parse($slider->updated_at)->diffForHumans() }}
                                        @endif
                                    </td>
                                    <td>

                                        <div class="dropdown show d-inline-block widget-dropdown">
                                            <a class="dropdown-toggle icon-burger-mini" href="#" role="button" id="dropdown-recent-order2" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" data-display="static"></a>
                                            <ul class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdown-recent-order2">
                                            <li class="dropdown-item">
                                                <a href="{{ route('edit.slider', $slider->id) }}">Edit</a>
                                            </li>
                                            <li class="dropdown-item">
                                                <a href="{{ route('destroy.slider', $slider->id) }}">Delete</a>
                                            </li>
                                            </ul>
                                        </div>

                                    </td>
                                </tr>
                                @endforeach
                                    
                            </tbody>
                        </table>
                    </div>
                </div>
            </div>
        </div>
    </div>

@endsection